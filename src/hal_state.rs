...
// We need to separate each column because using a matrix
// attribute that takes up multiple locations doesn't work on dx12
...
layout (location = 6) in vec4 uv_rect;
...
void main() {
    ...
    gl_Position = push.view_proj * model * vec4(
        position.x * (uv_rect.w/32), position.y, position.z * (uv_rect.z/32), 1.0
    );

    ivec2 texSize = textureSize(sampler2D(tex, samp), 0);
    frag_uv = vec2(
        (uv_rect.x + uv_rect.z*vert_uv.x)/float(texSize.x),
        (uv_rect.y + uv_rect.w*vert_uv.y)/float(texSize.y)
    );
}";
...
pub static CREATURE_BYTES: &[u8] = include_bytes!("img/spritesheet.png");
...
const QUAD_VERTEXES: [Vertex; 4] = [
    // Face 1 (front)
    Vertex { xyz: [-0.5, 0.0, -0.5], uv: [0.0, 1.0] }, /* bottom left */
    Vertex { xyz: [-0.5, 0.0,  0.5], uv: [0.0, 0.0] }, /* top left */
    Vertex { xyz: [ 0.5, 0.0, -0.5], uv: [1.0, 1.0] }, /* bottom right */
    Vertex { xyz: [ 0.5, 0.0,  0.5], uv: [1.0, 0.0] }, /* top right */
];

#[cfg_attr(rustfmt, rustfmt_skip)]
const QUAD_INDEXES: [u16; 6] = [
    0,  1,  2,  2,  1,  3, // front
];
...
pub struct HalState {
    quad_vertices: BufferBundle<back::Backend, <back::Backend as gfx_hal::Backend>::Device>,
    quad_instances: Vec<BufferBundle<back::Backend, <back::Backend as gfx_hal::Backend>::Device>>,
    quad_indexes: BufferBundle<back::Backend, <back::Backend as gfx_hal::Backend>::Device>,
    depth_images: Vec<DepthImage<back::Backend, <back::Backend as gfx_hal::Backend>::Device>>,
    texture: LoadedImage<back::Backend, <back::Backend as gfx_hal::Backend>::Device>,
    ...
    device: ManuallyDrop<<back::Backend as gfx_hal::Backend>::Device>,
...
impl HalState {
    ...
    pub fn new(window: &Window) -> Result<Self, &'static str> {
        // Create An Instance
        let instance = back::Instance::create("stockRPG", 1);
        ...
        unsafe {
            let mut data_target = device
                .acquire_mapping_writer(&quad_vertices.memory, 0..quad_vertices.requirements.size)
                .map_err(|_| "Failed to acquire an index buffer mapping writer!")?;
            data_target[..QUAD_VERTEXES.len()].copy_from_slice(&QUAD_VERTEXES);
            device
                .release_mapping_writer(data_target)
                .map_err(|_| "Couldn't release the index buffer mapping writer!")?;
        }

        let quad_indexes = BufferBundle::new(
            &adapter,
            &device,
        ...
        unsafe {
            let mut data_target = device
                .acquire_mapping_writer(&quad_indexes.memory, 0..quad_indexes.requirements.size)
                .map_err(|_| "Failed to acquire an index buffer mapping writer!")?;
            data_target[..QUAD_INDEXES.len()].copy_from_slice(&QUAD_INDEXES);
            device
                .release_mapping_writer(data_target)
                .map_err(|_| "Couldn't release the index buffer mapping writer!")?;
        }

        // Create an instance buffer per frame. We do this so that we can
        // write changes into one while another frame is in flight. We'll
        // write data into it later
    ...
    pub fn draw_appearances_frame(
        ...
        let quad_instance_buf = &mut self.quad_instances[i_usize];
...
impl core::ops::Drop for HalState {
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            self.quad_vertices.manually_drop(self.device.deref());
            self.quad_indexes.manually_drop(self.device.deref());
            for buf in self.quad_instances.drain(..) {
